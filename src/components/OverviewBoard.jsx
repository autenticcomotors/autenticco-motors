...
import { getPublicationsForCars, savePlatformsOrder } from '@/lib/car-api';
...
const OverviewBoard = ({ cars = [], platforms = [], onOpenGestaoForCar = () => {} }) => {
  ...
  // === NOVO: controle de edição de ordem de plataformas ===
  const [isEditingOrder, setIsEditingOrder] = useState(false);
  const [orderSaving, setOrderSaving] = useState(false);
  const [localPlatforms, setLocalPlatforms] = useState(() => [...platforms]);

  // se vier algo novo do banco, atualiza o local
  useEffect(() => {
    setLocalPlatforms([...platforms]);
  }, [platforms]);

  // extras fixos (mantive teus extras)
  ...
  // montar lista de colunas: plataformas do banco (AGORA usando localPlatforms) + extras
  const allColumnKeys = useMemo(() => {
    ...
    (localPlatforms || []).forEach((p) => {
      ...
      seen.add(nl);
      out.push({ key: `platform_${p.id}`, label, type: p.platform_type || 'other' });
    });
  ...
  // buscar publicações agrupadas por carro
  useEffect(() => {
    const run = async () => {
  ...
  const marketplaceCols = allColumnKeys.filter(
    (c) =>
      enabledCols[c.key] &&
      (c.type === 'marketplace' || (c.type === 'other' && /(mercado|olx|webmotor)/i.test(c.label)))
  );
  ...
  const csvData = useMemo(() => {
    ...
    const rows = (filteredCars || []).map((car) => {
      const row = [car.brand || '', car.model || '', car.year || '', car.price || '', car.plate || ''];
  ...
  // ====== FUNÇÕES DE ORDEM ======
  const marketplacePlatformsOnly = useMemo(
    () => (localPlatforms || []).filter((p) => p.platform_type === 'marketplace'),
    [localPlatforms]
  );
  const socialPlatformsOnly = useMemo(
    () => (localPlatforms || []).filter((p) => p.platform_type !== 'marketplace'),
    [localPlatforms]
  );
  ...
  const handleMoveMarketplace = (index, dir) => {
    const mps = move(marketplacePlatformsOnly, index, dir);
    const socials = socialPlatformsOnly;
    setLocalPlatforms([...mps, ...socials]);
  };

  const handleMoveSocial = (index, dir) => {
    const mps = marketplacePlatformsOnly;
    const socials = move(socialPlatformsOnly, index, dir);
    setLocalPlatforms([...mps, ...socials]);
  };
  ...
  const handleSaveOrder = async () => {
    try {
      setOrderSaving(true);
      const body = (localPlatforms || []).map((p) => ({ id: p.id }));
      const res = await savePlatformsOrder(body);
      if (res?.error) {
        toast({
        ...
        toast({
          title: 'Ordem salva',
          description: 'A nova ordem foi gravada no banco.',
        });
  ...
  return (
    <div className="w-full">
      ...
      <div className="bg-white rounded-lg border p-4 mb-4">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              ...
              <div className="ml-auto flex items-center gap-2">
                <button
                  ...
                  className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded bg-yellow-100 text-yellow-900 border border-yellow-200"
                >
                  <GripVertical className="w-3 h-3" />
                  Editar ordem
                </button>
                <div className="text-xs text-gray-400">Seleção salva localmente</div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2 max-h-[72px] overflow-hidden">
              ...
              <button onClick={resetColumns} className="text-xs px-2 py-1 rounded-full border border-dashed text-gray-600">
                Reset
              </button>
            </div>
          </div>
        ...
        {/* painel de edição de ordem (aparece só quando está editando) */}
        {isEditingOrder && (
          <div className="mt-4 bg-gray-50 border rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              <p className="text-sm font-semibold text-gray-700">
                Ordenar plataformas (salva no banco)
              </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-white rounded-lg border">
                <div className="px-3 py-2 border-b bg-amber-50 text-sm font-semibold">
                  Anúncios / Marketplaces
                </div>
                <div>
                  {marketplacePlatformsOnly.map((p, idx) => (
                    <div key={p.id} className="flex items-center justify-between px-3 py-2 gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => handleMoveMarketplace(idx, +1)}
                          disabled={idx === marketplacePlatformsOnly.length - 1}
                          className={`p-1 rounded ${
                            idx === marketplacePlatformsOnly.length - 1
                              ? 'text-gray-300'
              ...
              {/* socials */}
              <div className="bg-white rounded-lg border">
                <div className="px-3 py-2 border-b bg-blue-50 text-sm font-semibold">
                  Redes sociais
                </div>
                <div>
                  {socialPlatformsOnly.map((p, idx) => (
                    <div key={p.id} className="flex items-center justify-between px-3 py-2 gap-2">
                      <div className="flex items-center gap-2">
                        <button
                        ...
                        <button
                          onClick={() => handleMoveSocial(idx, +1)}
                          disabled={idx === socialPlatformsOnly.length - 1}
                          className={`p-1 rounded ${
                            idx === socialPlatformsOnly.length - 1
                              ? 'text-gray-300'
                              : 'text-gray-700 hover:bg-gray-100'
                          }`}
                        >
      ...
      <div className="bg-white rounded-lg border overflow-auto max-h-[70vh]">
        <table className="w-full border-collapse min-w-[1100px]">
          <thead className="sticky top-0 z-20">
            ...
            <tr className="text-xs text-gray-600 sticky top-[48px] z-20">
              ...
              filteredCars.map((car, idx) => {
                ...
                return (
                  <tr key={car.id} className={`border-t ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-500 mt-1">
                            {car.year ? `${car.year}` : ''} {car.is_blindado ? ' • BLINDADO' : ''}
                          </div>
                          {car.updated_at && (
                    ...
                    <td className="p-4 align-top">
                      <div className="text-sm font-medium text-gray-700">{car.plate || '-'}</div>
                    </td>
